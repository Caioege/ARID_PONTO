...
namespace AriD.GerenciamentoDePonto.Controllers
{
    ...
    public class RastreioAppController : Controller
    {
        ...
        [HttpGet("rotas/trajeto-execucao")]
        public IActionResult TrajetoExecucao(int rotaExecucaoId, [FromHeader(Name = "Authorization")] string auth)
        {
            int motoristaId = ObterMotoristaId(auth);
            if (motoristaId == 0) return Unauthorized();

            var pontos = _servicoDeRastreio.ObterTrajetoExecucao(rotaExecucaoId, motoristaId);
            return Ok(new { data = pontos });
        }
        ...
        [HttpGet("rotas/finalizada-do-dia")]
        public IActionResult ObterFinalizadaDoDia(int rotaId, [FromHeader(Name = "Authorization")] string auth)
        {
            int motoristaId = ObterMotoristaId(auth);
            if (motoristaId == 0) return Unauthorized();

            var retorno = _servicoDeRastreio.ObterRotaFinalizadaDoDia(motoristaId, rotaId);
            return Ok(retorno);
        }
        ...
        [HttpGet("rotas/chat")]
        public IActionResult ObterChat(int rotaExecucaoId, [FromHeader(Name = "Authorization")] string auth)
        {
            int servidorId = ObterServidorId(auth);
            if (servidorId <= 0) return Unauthorized();

            var resumo = _servicoChatRota.ObterChatAplicativo(rotaExecucaoId, servidorId);
            return Ok(resumo);
        }

        [HttpGet("rotas/chat/nao-lidas")]
        public IActionResult ObterNaoLidasChat(int rotaExecucaoId, [FromHeader(Name = "Authorization")] string auth)
        {
            int servidorId = ObterServidorId(auth);
            if (servidorId <= 0) return Unauthorized();

            var quantidade = _servicoChatRota.ObterNaoLidasAplicativo(rotaExecucaoId, servidorId);
            return Ok(new { sucesso = true, quantidade });
        }

        [HttpPost("rotas/chat")]
        public IActionResult EnviarMensagemChat([FromBody] RotaChatEnvioDTO dto, [FromHeader(Name = "Authorization")] string auth)
        {
            int servidorId = ObterServidorId(auth);
            if (servidorId <= 0) return Unauthorized();

            var mensagem = _servicoChatRota.EnviarMensagemAplicativo(dto.RotaExecucaoId, servidorId, dto.Mensagem);
            return Ok(new { sucesso = true, data = mensagem });
        }
