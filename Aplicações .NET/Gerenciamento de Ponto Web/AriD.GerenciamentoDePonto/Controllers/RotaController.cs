...
namespace AriD.GerenciamentoDePonto.Controllers
{
    public class RotaController : BaseController
    {
        ...
        private readonly IServicoChatRota _servicoChatRota;
        ...
        public RotaController(
            ...
            IServicoChatRota servicoChatRota,
            IServicoNotificacao servicoNotificacao)
        {
            ...
            _servicoChatRota = servicoChatRota;
        ...
        public async Task<IActionResult> Salvar(Rota rota, List<ParadaRota> paradas, List<int> veiculosSelecionados, string pacientesJson, string profissionaisJson)
        {
            ...
            else
            {
                ...
                original.PermiteIniciarSemPacienteAcompanhante = rota.PermiteIniciarSemPacienteAcompanhante;
                original.PermiteIniciarSemProfissional = rota.PermiteIniciarSemProfissional;
        ...
        public IActionResult ObtenhaHistoricoExecucoes(int rotaId)
        {
            var historico = _rotaExecucaoServico.ObtenhaLista(h => h.RotaId == rotaId)
                ...
                .Select(h => new {
                    ...
                    ChecklistExecucaoId = h.ChecklistExecucaoId
                });
            return Json(new { sucesso = true, historico = historico });
        }

        [HttpGet]
        public IActionResult ObterChecklistExecucao(int execucaoId)
        {
            try
            {
                if (!HttpContext.PossuiPermissao(eItemDePermissao_MonitoramentoMobile.Visualizar)
                    && !HttpContext.PossuiPermissao(eItemDePermissao_Rota.Visualizar))
                    throw new ApplicationException("Você não tem permissão para consultar o checklist da execução.");

                var organizacaoId = this.HttpContext.DadosDaSessao().OrganizacaoId;
                var checklist = _servicoMonitoramentoRotas.ObtenhaChecklistExecucao(organizacaoId, execucaoId);

                if (checklist == null)
                    return Json(new { sucesso = false, mensagem = "Nenhum checklist foi localizado para esta execução de rota." });

                return Json(new { sucesso = true, dados = checklist });
            }
            catch (Exception ex)
            {
        ...
        public IActionResult ObterDadosMonitoramento(string dataFiltro, bool exibirFinalizadas = false)
        {
            try
            {
                ...
                var resultadoMonitoramento = _servicoMonitoramentoRotas.ObtenhaMonitoramento(organizacaoId, dataBase, exibirFinalizadas);
                var resultado = resultadoMonitoramento.Rotas.ToList();
                ...
                return Json(new { sucesso = true, dados = resultado, manutencoesVeiculos = resultadoMonitoramento.ManutencoesVeiculos });
            }
            catch (Exception ex)
            {
                return Json(new { sucesso = false, mensagem = ex.Message });
            ...
            try
            {
                ...
                var eventosAuditoria = _rotaExecucaoEventoServico.ObtenhaLista(ev => ev.RotaExecucaoId == exec.Id)
                    ...
                    .Select(ev => new
                    {
                        ...
                        ev.Entregue,
                        ev.Observacao,
                        Latitude = string.IsNullOrWhiteSpace(ev.Latitude) ? null : ev.Latitude,
                        Longitude = string.IsNullOrWhiteSpace(ev.Longitude) ? null : ev.Longitude,
                ...
                var resultado = new
                {
                    ...
                    HoraFim = exec.DataHoraFim?.ToString("dd/MM/yyyy HH:mm:ss"),
                    ObservacaoInicio = exec.ObservacaoInicio,
                    ObservacaoFim = exec.ObservacaoFim,
        ...
        [HttpGet]
        public IActionResult ObterChatRota(int execucaoId)
        {
            try
            {
                var organizacaoId = this.HttpContext.DadosDaSessao().OrganizacaoId;
                var chat = _servicoChatRota.ObterChat(organizacaoId, execucaoId);
                return Json(new { sucesso = true, dados = chat });
            }
            catch (Exception ex)
            {
                return Json(new { sucesso = false, mensagem = ex.Message });
        ...
        [HttpPost]
        public IActionResult ObterNaoLidasChatRotas([FromBody] List<int> execucoesIds)
        {
            try
            {
                var organizacaoId = this.HttpContext.DadosDaSessao().OrganizacaoId;
                var naoLidas = _servicoChatRota.ObterNaoLidasSistema(organizacaoId, execucoesIds ?? new List<int>());
                return Json(new { sucesso = true, dados = naoLidas });
            }
            catch (Exception ex)
            {
                return Json(new { sucesso = false, mensagem = ex.Message });
        ...
        [HttpPost]
        public async Task<IActionResult> EnviarMensagemChatRota([FromBody] RotaChatEnvioDTO dto)
        {
            try
            {
                var sessao = this.HttpContext.DadosDaSessao();
                var mensagem = _servicoChatRota.EnviarMensagemSistema(sessao.OrganizacaoId, dto.RotaExecucaoId, sessao.UsuarioId, sessao.UsuarioNome ?? "Sistema", dto.Mensagem);

                var destinoPush = _servicoChatRota.ObterDestinoPushAplicativo(sessao.OrganizacaoId, dto.RotaExecucaoId);
                var pushEnviado = false;

                if (destinoPush != null && !string.IsNullOrWhiteSpace(destinoPush.PushToken))
                {
                    pushEnviado = await _servicoNotificacao.EnviarNotificacaoPush(
                        destinoPush.PushToken,
                        $"Chat da rota {destinoPush.RotaDescricao}",
                        mensagem.Mensagem,
                        new
                        {
                            tipo = "rota_chat",
                            rotaExecucaoId = dto.RotaExecucaoId,
                            rotaId = destinoPush.RotaId
                        });
                }

                return Json(new { sucesso = true, dados = mensagem, pushEnviado });
            }
            catch (Exception ex)
            {
                return Json(new { sucesso = false, mensagem = ex.Message });
