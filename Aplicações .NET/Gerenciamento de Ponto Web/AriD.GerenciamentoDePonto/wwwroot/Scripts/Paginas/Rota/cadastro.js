function assineEventosCadastro() {
    ...
    $('#btn-salvar-cadastro').on('click', function () {
        ...
        payload['PermiteIniciarSemPacienteAcompanhante'] = $('#PermiteIniciarSemPacienteAcompanhante').is(':checked');
        payload['PermiteIniciarSemProfissional'] = $('#PermiteIniciarSemProfissional').is(':checked');
...
function carregarHistoricoExecucoes() {
    let id = $('#Id').val();
    $('#lista-historico-execucoes').html('<div class="route-history-empty"><i class="bx bx-loader bx-spin"></i> Carregando execuções...</div>');
    $.ajax({
        ...
        success: function (data) {
            if (data.sucesso && data.historico) {
                renderizarListaHistoricoExecucoes(data.historico);
                return;
...
function renderizarListaHistoricoExecucoes(historico) {
    let lista = $('#lista-historico-execucoes');
    let tbody = $('#tabela-historico-execucoes tbody');
    lista.empty();
    tbody.empty();

    if (!historico || historico.length === 0) {
        lista.html('<div class="route-history-empty">Nenhuma execução registrada para esta rota.</div>');
        tbody.append('<tr><td colspan="6" class="text-center">Nenhuma execução registrada.</td></tr>');
        return;
    }

    historico.forEach(function (h) {
        let botaoMapa = `<button type="button" class="btn btn-sm btn-icon btn-primary" title="Ver rota" onclick="visualizarRotaNoMapa(${h.id})"><i class='bx bx-map-alt'></i></button>`;
        let botaoChecklist = h.checklistExecucaoId
            ? `<button type="button" class="btn btn-sm btn-icon btn-outline-dark ms-1" title="Ver checklist" onclick="abrirChecklistExecucaoHistorico(${h.id}, event)"><i class='bx bx-list-check'></i></button>`
            : '';
        let auditoria = montarBadgeOfflineHistorico(h);
        tbody.append(`<tr><td>${h.dataHoraInicio}</td><td>${h.usuarioInicio}</td><td>${h.dataHoraFim || '-'}</td><td>${h.usuarioFim || '-'}</td><td>${auditoria}</td><td>${botaoMapa}${botaoChecklist}</td></tr>`);
        lista.append(montarCardHistoricoExecucao(h, auditoria));
    });
}

function montarCardHistoricoExecucao(h, auditoria) {
    let finalizada = h.dataHoraFim && h.dataHoraFim !== 'Em andamento';
    ...
    let botaoChecklist = h.checklistExecucaoId
        ? `<button type="button" class="btn btn-sm btn-outline-dark" onclick="abrirChecklistExecucaoHistorico(${h.id}, event)"><i class="bx bx-list-check"></i> Checklist</button>`
        : '<span class="badge bg-label-secondary">Sem checklist</span>';

    return `
        <div class="route-history-card text-start" role="button" tabindex="0" onclick="visualizarRotaNoMapa(${h.id})">
            <div class="d-flex justify-content-between align-items-start gap-2 mb-2">
                <strong>Execução #${h.id}</strong>
                ${status}
            </div>
            <span><i class="bx bx-play-circle"></i> Início: ${escapeHtmlCadastro(h.dataHoraInicio || '-')}</span>
            <span><i class="bx bx-flag"></i> Fim: ${escapeHtmlCadastro(h.dataHoraFim || '-')}</span>
            <span><i class="bx bx-user"></i> Iniciada por: ${escapeHtmlCadastro(h.usuarioInicio || '-')}</span>
...
function abrirChecklistExecucaoHistorico(execucaoId, event) {
    ...
    $('#checklist-modal-conteudo').html('<div class="route-history-empty">Carregando checklist...</div>');

    let modalEl = document.getElementById('modal-checklist-execucao');
    ...
    $.getJSON('/Rota/ObterChecklistExecucao', { execucaoId: execucaoId }, function (response) {
        if (!response || !response.sucesso) {
            $('#checklist-modal-conteudo').html('<div class="route-history-empty">' + escapeHtmlCadastro((response && response.mensagem) || 'Não foi possível carregar o checklist desta execução.') + '</div>');
            return;
        }

        renderizarChecklistExecucaoHistorico(response.dados);
    }).fail(function () {
        $('#checklist-modal-conteudo').html('<div class="route-history-empty">Não foi possível carregar o checklist desta execução.</div>');
    });
}

function renderizarChecklistExecucaoHistorico(checklist) {
    checklist = checklist || {};
    let itens = checklist.itens || [];
    let resumo = (checklist.totalMarcados || 0) + ' de ' + (checklist.totalItens || itens.length || 0) + ' itens marcados';
    ...
    let itensHtml = itens.length === 0
        ? '<div class="route-history-empty">Nenhum item de checklist cadastrado para o veículo desta execução.</div>'
        : '<div class="list-group list-group-flush border rounded">' + itens.map(function (item) {
            let classe = item.marcado ? 'text-success' : 'text-danger';
            let icone = item.marcado ? 'bx-check-circle' : 'bx-x-circle';
            let texto = item.marcado ? 'Marcado' : 'Não marcado';

            return '<div class="list-group-item d-flex justify-content-between align-items-center gap-2">' +
                '<span>' + escapeHtmlCadastro(item.descricao || '-') + '</span>' +
                '<span class="' + classe + ' fw-bold text-nowrap"><i class="bx ' + icone + '"></i> ' + texto + '</span>' +
                '</div>';
        }).join('') + '</div>';

    $('#checklist-modal-conteudo').html(
        '<div class="route-history-detail-grid mb-3">' +
        detailHistoricoItem('Data/hora', checklist.dataHora || '-') +
        detailHistoricoItem('Motorista', checklist.motoristaNome || '-') +
        detailHistoricoItem('Veículo', checklist.veiculoDescricao || '-') +
        detailHistoricoItem('Resumo', resumo) +
        '</div>' +
        itensHtml
    );
}
            ...
            RequisicaoAjaxComCarregamento(
                ...
                function (res) {
                    if (res.sucesso && res.dados.length > 0) {
                        ...
                        setTimeout(function () {
                            if (historicoMap) historicoMap.invalidateSize();
                        }, 150);
                        return;
                        let hexBase = "#34495e";
...
function renderizarHistoricoAtual() {
    ...
    atualizarPainelHistoricoExecucao(rota, filtro);
...
function atualizarPainelHistoricoExecucao(rota, filtro) {
    let finalizada = rota.finalizada === true;
    $('#historico-rota-titulo').text(rota.descricao || 'Execução da rota');
    $('#historico-rota-subtitulo').text((rota.placaModelo || 'Veículo não informado') + ' | ' + (rota.motoristaNome || 'Motorista não informado'));
    $('#historico-rota-status')
        .attr('class', 'route-history-status ' + (finalizada ? 'finalizada' : 'andamento'))
        .text(finalizada ? 'Finalizada' : 'Em andamento');

    $('#historico-detalhes-rota').html(
        detailHistoricoItem('Motorista', rota.motoristaNome || '-') +
        detailHistoricoItem('Veículo', rota.placaModelo || '-') +
        detailHistoricoItem('Início', rota.horaInicio || '-') +
        detailHistoricoItem('Fim', rota.horaFim || '-') +
        detailHistoricoItem('Última atualização', rota.ultimaAtualizacao || '-') +
        detailHistoricoItem('Observação início', rota.observacaoInicio || '-') +
        detailHistoricoItem('Observação fim', rota.observacaoFim || '-')
    );

    $('#historico-linha-tempo').html(montarLinhaTempoHistorico(rota, filtro));
}

function detailHistoricoItem(titulo, valor) {
    return `<div class="route-history-detail-item"><small>${escapeHtmlCadastro(titulo)}</small><span>${escapeHtmlCadastro(valor || '-')}</span></div>`;
}

function montarLinhaTempoHistorico(rota, filtro) {
    let itens = [];

    if (rota.horaInicio) {
        itens.push({
            ordem: extrairDataOrdenacaoCadastro(rota.horaInicio),
            titulo: 'Rota iniciada',
            detalhe: rota.horaInicio,
            tipo: 'inicio',
            icone: 'bx-play'
        });
    }

    (rota.eventosAuditoria || [])
        .filter(e => filtro === 'todos' || (filtro === 'offline' ? e.registradoOffline === true : e.registradoOffline !== true))
        .forEach(function (e) {
            if (e.tipoEvento === 1) return;
            if (e.tipoEvento === 5) return;

            itens.push({
                ordem: extrairDataOrdenacaoCadastro(e.dataHora),
                titulo: obterTituloEventoHistorico(e),
                detalhe: montarDetalheEventoHistorico(e),
                tipo: obterTipoVisualEventoHistorico(e),
                icone: obterIconeEventoHistorico(e)
            });
        });

    (rota.pausasAuditoria || [])
        .filter(p => filtro === 'todos' || (filtro === 'offline' ? p.registradoOffline === true : p.registradoOffline !== true))
        .forEach(function (p) {
            itens.push({
                ordem: extrairDataOrdenacaoCadastro(p.dataHoraInicio),
                titulo: 'Pausa',
                detalhe: (p.dataHoraInicio || '-') + (p.dataHoraFim ? ' até ' + p.dataHoraFim : '') + (p.motivo ? ' | ' + p.motivo : ''),
                tipo: 'evento',
                icone: 'bx-pause'
            });
        });

    if (rota.finalizada === true && rota.horaFim) {
        itens.push({
            ordem: extrairDataOrdenacaoCadastro(rota.horaFim),
    ...
    if (itens.length === 0) {
        return '<div class="route-history-empty">Nenhum evento registrado.</div>';
    }

    return itens.map(function (item) {
        return `<div class="route-history-timeline-item">
            <div class="route-history-timeline-dot ${escapeAttrCadastro(item.tipo || 'evento')}"><i class="bx ${escapeAttrCadastro(item.icone || 'bx-circle')}"></i></div>
            <div class="route-history-timeline-content">
                <strong>${escapeHtmlCadastro(item.titulo)}</strong>
                <span>${escapeHtmlCadastro(item.detalhe || '-')}</span>
            </div>
        </div>`;
    }).join('');
}

function obterTituloEventoHistorico(evento) {
    if (!evento) return 'Evento';
    switch (evento.tipoEvento) {
        case 2: return 'Unidade de origem';
        case 3: return 'Parada confirmada';
        case 4: return 'Unidade destino';
        case 5: return 'Rota finalizada';
        default: return evento.tipoDescricao || 'Evento';
    }
}

function obterTipoVisualEventoHistorico(evento) {
    if (!evento) return 'evento';
    if (evento.tipoEvento === 3) return evento.entregue === false ? 'parada-nao-realizada' : 'parada-realizada';
    if (evento.tipoEvento === 5) return 'fim';
    return 'evento';
}

function obterIconeEventoHistorico(evento) {
    if (!evento) return 'bx-circle';
    if (evento.tipoEvento === 2 || evento.tipoEvento === 4) return evento.entregue === false ? 'bx-x' : 'bx-check';
    if (evento.tipoEvento === 3) return evento.entregue === false ? 'bx-x' : 'bx-check';
    if (evento.tipoEvento === 5) return 'bxs-flag-checkered';
    return 'bxs-bell-ring';
}

function montarDetalheEventoHistorico(evento) {
    let partes = [];
    if (evento.dataHora) partes.push(evento.dataHora);
    if (evento.entregue !== null && evento.entregue !== undefined) partes.push(evento.entregue ? 'Realizada' : 'Não realizada');
    if (evento.observacao) partes.push(evento.observacao);
    if (evento.registradoOffline) partes.push('offline');
    return partes.join(' | ');
}

function extrairDataOrdenacaoCadastro(valor) {
    if (!valor) return 0;
    ...
    match = texto.match(/^(\d{2})\/(\d{2})\/(\d{2})\s+(\d{2}):(\d{2})(?::(\d{2}))?/);
    if (match) {
        return new Date(2000 + Number(match[3]), Number(match[2]) - 1, Number(match[1]), Number(match[4]), Number(match[5]), Number(match[6] || 0)).getTime();
    }
    let parsed = Date.parse(texto);
...
function escapeAttrCadastro(value) {
    return escapeHtmlCadastro(value).replace(/`/g, '&#96;');
}
