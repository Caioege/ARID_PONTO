...
namespace AriD.Servicos.Servicos
{
    public class ServicoDeAplicativoDeRastreio : IServicoDeAplicativoDeRastreio
    {
        ...
        public List<RotaCheckListDTO> ObterRotasMotorista(int motoristaId)
        {
            var sql = @"
                SELECT r.Id, r.Descricao as Nome, r.Recorrente, r.DataParaExecucao, r.DataInicio, r.DataFim, r.DiasSemana,
                       r.PermiteIniciarSemPacienteAcompanhante, r.PermiteIniciarSemProfissional
                FROM rota r
            ...
            var execucoesFinalizadasPorRota = ObterExecucoesFinalizadasHojePorRota(motoristaId);
            var organizacaoId = ObterOrganizacaoMotorista(motoristaId);
            var pacientesDisponiveis = ObterPacientesDisponiveis(organizacaoId);
            ...
            return todas
                ...
                .Select(r => new RotaCheckListDTO
                {
                    ...
                    RotaFinalizada = execucoesFinalizadasPorRota.ContainsKey(r.Id),
                    RotaExecucaoFinalizadaId = execucoesFinalizadasPorRota.TryGetValue(r.Id, out var execucaoId) ? execucaoId : null,
                    PermiteIniciarSemPacienteAcompanhante = r.PermiteIniciarSemPacienteAcompanhante,
                    PermiteIniciarSemProfissional = r.PermiteIniciarSemProfissional,
                    Pacientes = ObterPacientesDaRota(r.Id),
                    Profissionais = ObterProfissionaisDaRota(r.Id),
        ...
        public PacoteOfflineRastreioDTO ObterPacoteOfflineMotorista(int motoristaId)
        {
            ...
            var sql = @"
                SELECT
                    ...
                    r.PermitePausa,
                    r.QuantidadePausas,
                    r.PermiteIniciarSemPacienteAcompanhante,
                    r.PermiteIniciarSemProfissional,
                    r.UnidadeOrigemId,
                    r.UnidadeDestinoId,
            ...
            foreach (var rota in rotasValidas)
            {
                pacote.Rotas.Add(new RotaOfflineDTO
                {
                    ...
                    PermiteIniciarSemPacienteAcompanhante = rota.PermiteIniciarSemPacienteAcompanhante,
                    PermiteIniciarSemProfissional = rota.PermiteIniciarSemProfissional,
                    ...
                    Pacientes = ObterPacientesDaRota(rota.Id),
                    Profissionais = ObterProfissionaisDaRota(rota.Id),
                    PacientesDisponiveis = pacientesDisponiveis
                });
            }
            ...
            try
            {
                var rota = conn.QueryFirstOrDefault<RotaSincronizacaoOfflineLookupDTO>(
                    @"SELECT Id, OrganizacaoId, UnidadeOrigemId, UnidadeDestinoId,
                             PermiteIniciarSemPacienteAcompanhante, PermiteIniciarSemProfissional
                      FROM rota
                      WHERE Id = @RotaId",
                    new { dto.RotaId },
                    transaction);
                ...
                if (!execucaoId.HasValue)
                {
                    ...
                    ValidarPresencasParaInicio(rota, dto.PacientesPresenca, dto.ProfissionaisPresenca);
                ...
                SalvarPresencasInicioRota(conn, transaction, execucaoId.Value, rota.OrganizacaoId, dto.RotaId, dto.PacientesPresenca, dto.ProfissionaisPresenca, agora);
        ...
        private Dictionary<int, int> ObterExecucoesFinalizadasHojePorRota(int motoristaId)
        {
            var hoje = DateTime.Now.Date;
            var amanha = hoje.AddDays(1);

            var sql = @"
                SELECT re.RotaId, MAX(re.Id) as ExecucaoId
                FROM rotaexecucao re
                INNER JOIN rota r ON r.Id = re.RotaId
                WHERE re.MotoristaId = @MotoristaId
                  AND re.Status = @StatusFinalizada
                  AND re.DataHoraFim >= @Hoje
                  AND re.DataHoraFim < @Amanha
                  AND (
                        r.Recorrente = 1
                        OR (r.Recorrente = 0 AND r.DataParaExecucao >= @Hoje AND r.DataParaExecucao < @Amanha)
                      )
                GROUP BY re.RotaId";

            return _repositorioExecucao.ConsultaDapper<RotaExecucaoFinalizadaHojeRowDTO>(sql, new
                {
                    MotoristaId = motoristaId,
                ...
                .ToDictionary(r => r.RotaId, r => r.ExecucaoId);
        }
        ...
        public IEnumerable<UltimaLocalizacaoRotaDTO> ObterTrajetoExecucao(int rotaExecucaoId, int motoristaId)
        {
            var sql = @"
                SELECT l.Latitude, l.Longitude, l.DataHoraCaptura as DataHora
                FROM rotaexecucao re
                INNER JOIN rotaexecucaolocalizacao l ON l.RotaExecucaoId = re.Id
                WHERE re.Id = @ExecucaoId
                  AND re.MotoristaId = @MotoristaId
                ORDER BY l.DataHoraCaptura, l.Id";

            return _repositorioLocalizacao.ConsultaDapper<UltimaLocalizacaoRotaDTO>(sql, new
            {
                ExecucaoId = rotaExecucaoId,
                MotoristaId = motoristaId
            });
        }
        ...
        private int ObterOrganizacaoMotorista(int motoristaId)
        {
            var sql = "SELECT OrganizacaoId FROM motorista WHERE Id = @MotoristaId";
            return _repositorioMotorista.ConsultaDapper<int>(sql, new { MotoristaId = motoristaId }).FirstOrDefault();
        }

        private List<RotaPacienteAppDTO> ObterPacientesDaRota(int rotaId)
        {
            var sql = @"
                SELECT p.Id as PacienteId, p.Nome, p.CPF, p.Telefone, rp.PossuiAcompanhante, p.AcompanhanteNome, p.AcompanhanteCPF
                FROM rotapaciente rp
                INNER JOIN paciente p ON p.Id = rp.PacienteId
                WHERE rp.RotaId = @RotaId
                ORDER BY p.Nome";

            return _repositorioRota.ConsultaDapper<RotaPacienteAppDTO>(sql, new { RotaId = rotaId }).ToList();
        }

        private List<PacienteDisponivelAppDTO> ObterPacientesDisponiveis(int organizacaoId)
        {
            if (organizacaoId <= 0) return new List<PacienteDisponivelAppDTO>();

            var sql = @"
                SELECT Id as PacienteId, Nome, CPF, Telefone, AcompanhanteNome, AcompanhanteCPF
                FROM paciente
                WHERE OrganizacaoId = @OrganizacaoId
                  AND Ativo = 1
                ORDER BY Nome";

            return _repositorioRota.ConsultaDapper<PacienteDisponivelAppDTO>(sql, new { OrganizacaoId = organizacaoId }).ToList();
        }

        private List<RotaProfissionalAppDTO> ObterProfissionaisDaRota(int rotaId)
        {
            var sql = @"
                SELECT s.Id as ServidorId, p.Nome, COALESCE(f.Descricao, 'Profissional') as Funcao, rp.Observacao
                FROM rotaprofissional rp
                INNER JOIN servidor s ON s.Id = rp.ServidorId
                INNER JOIN pessoa p ON p.Id = s.PessoaId
                LEFT JOIN vinculodetrabalho vt ON vt.ServidorId = s.Id
                LEFT JOIN funcao f ON f.Id = vt.FuncaoId
                WHERE rp.RotaId = @RotaId
                GROUP BY s.Id, p.Nome, f.Descricao, rp.Observacao
                ORDER BY p.Nome";

            return _repositorioRota.ConsultaDapper<RotaProfissionalAppDTO>(sql, new { RotaId = rotaId }).ToList();
        }

        private void SalvarPresencasInicioRota(int execucaoId, IniciarRotaAppDTO dto, DateTime agora)
        {
            using var conn = _repositorioExecucao.MySQLConn();
            conn.Open();
            using var transaction = conn.BeginTransaction();
            ...
            try
            {
                var rota = conn.QueryFirst<RotaExecucaoPresencaLookupDTO>(
                    "SELECT RotaId, OrganizacaoId FROM rotaexecucao WHERE Id = @ExecucaoId",
                    new { ExecucaoId = execucaoId },
                    transaction);

                SalvarPresencasInicioRota(conn, transaction, execucaoId, rota.OrganizacaoId, rota.RotaId, dto.PacientesPresenca, dto.ProfissionaisPresenca, agora);
                transaction.Commit();
            }
            catch
            {
        ...
        private void SalvarPresencasInicioRota(
            System.Data.IDbConnection conn,
            System.Data.IDbTransaction transaction,
            int execucaoId,
            int organizacaoId,
            int rotaId,
            List<PresencaPacienteRotaAppDTO> pacientes,
            List<PresencaProfissionalRotaAppDTO> profissionais,
            DateTime agora)
        {
            conn.Execute("DELETE FROM rotaexecucaopresenca WHERE RotaExecucaoId = @ExecucaoId", new { ExecucaoId = execucaoId }, transaction);

            foreach (var paciente in pacientes ?? new List<PresencaPacienteRotaAppDTO>())
            {
                var pacienteId = ObterOuCriarPacientePresenca(conn, transaction, organizacaoId, rotaId, paciente, agora);
                if (!pacienteId.HasValue) continue;

                InserirPresenca(conn, transaction, organizacaoId, execucaoId, rotaId, 1, pacienteId, null, paciente.Nome, paciente.CPF, paciente.Telefone, paciente.Presente, paciente.NovoPaciente, agora);

                if (paciente.PossuiAcompanhante || !string.IsNullOrWhiteSpace(paciente.AcompanhanteNome))
                {
                    InserirPresenca(conn, transaction, organizacaoId, execucaoId, rotaId, 2, pacienteId, null, paciente.AcompanhanteNome, paciente.AcompanhanteCPF, null, paciente.AcompanhantePresente, paciente.NovoPaciente, agora);
                }
            }

            foreach (var profissional in profissionais ?? new List<PresencaProfissionalRotaAppDTO>())
            {
                InserirPresenca(conn, transaction, organizacaoId, execucaoId, rotaId, 3, null, profissional.ServidorId, profissional.Nome, null, null, profissional.Presente, false, agora);
            }
        }

        private int? ObterOuCriarPacientePresenca(
            System.Data.IDbConnection conn,
            System.Data.IDbTransaction transaction,
            int organizacaoId,
            int rotaId,
            PresencaPacienteRotaAppDTO paciente,
            DateTime agora)
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(paciente.Nome)) return null;

                pacienteId = conn.QuerySingle<int>(
                    @"INSERT INTO paciente
                        (OrganizacaoId, Nome, CPF, Telefone, AcompanhanteNome, AcompanhanteCPF, Ativo, DataInclusao)
                      VALUES
                        (@OrganizacaoId, @Nome, @CPF, @Telefone, @AcompanhanteNome, @AcompanhanteCPF, 1, @Agora);
                      SELECT LAST_INSERT_ID();",
                    new
                    {
                        ...
                        paciente.CPF,
                        paciente.Telefone,
                        paciente.AcompanhanteNome,
                        paciente.AcompanhanteCPF,
                        Agora = agora
                    },
                    transaction);
            }

            var vinculoExiste = conn.QueryFirstOrDefault<int>(
                "SELECT Id FROM rotapaciente WHERE RotaId = @RotaId AND PacienteId = @PacienteId LIMIT 1",
                new { RotaId = rotaId, PacienteId = pacienteId.Value },
                transaction);

            if (vinculoExiste == 0)
            {
                conn.Execute(
                    @"INSERT INTO rotapaciente (RotaId, PacienteId, PossuiAcompanhante)
                      VALUES (@RotaId, @PacienteId, @PossuiAcompanhante)",
                    new { RotaId = rotaId, PacienteId = pacienteId.Value, paciente.PossuiAcompanhante },
                    transaction);
            }
        ...
        {
            conn.Execute(
                @"INSERT INTO rotaexecucaopresenca
                    (OrganizacaoId, RotaExecucaoId, RotaId, TipoParticipante, PacienteId, ServidorId, Nome, Documento, Telefone, Presente, CriadoPeloApp, DataHoraRegistro)
                  VALUES
                    (@OrganizacaoId, @ExecucaoId, @RotaId, @TipoParticipante, @PacienteId, @ServidorId, @Nome, @Documento, @Telefone, @Presente, @CriadoPeloApp, @Agora)",
                new
                {
                    OrganizacaoId = organizacaoId,
        ...
        public RotaExecucaoDTO IniciarRota(IniciarRotaAppDTO dto, int motoristaId)
        {
            ...
            ValidarPresencasParaInicio(dto.RotaId, dto.PacientesPresenca, dto.ProfissionaisPresenca);
            ...
            SalvarPresencasInicioRota(execucaoId, dto, agora);
        ...
        public RotaExecucaoDTO? ObterRotaFinalizadaDoDia(int motoristaId, int rotaId)
        {
            var hoje = DateTime.Now.Date;
            var amanha = hoje.AddDays(1);
            ...
            var sql = @"
                ...
                ORDER BY re.DataHoraFim DESC, re.Id DESC
                LIMIT 1";

            var execucaoId = _repositorioExecucao.ConsultaDapper<int>(sql, new
            {
                MotoristaId = motoristaId,
                RotaId = rotaId,
                StatusFinalizada = StatusExecucaoFinalizada,
        ...
        private void ValidarPresencasParaInicio(int rotaId, List<PresencaPacienteRotaAppDTO> pacientes, List<PresencaProfissionalRotaAppDTO> profissionais)
        {
            var rota = _repositorioRota.ConsultaDapper<ParametrosInicioRotaDTO>(
                @"SELECT Id, PermiteIniciarSemPacienteAcompanhante, PermiteIniciarSemProfissional
                  FROM rota
                  WHERE Id = @RotaId",
                new { RotaId = rotaId })
                .FirstOrDefault();
            ...
            if (rota == null)
                throw new ApplicationException("Rota informada nao existe.");

            ValidarPresencasParaInicio(rota, pacientes, profissionais);
        }

        private void ValidarPresencasParaInicio(ParametrosInicioRotaDTO rota, List<PresencaPacienteRotaAppDTO> pacientes, List<PresencaProfissionalRotaAppDTO> profissionais)
        {
            var temPacienteOuAcompanhantePresente = (pacientes ?? new List<PresencaPacienteRotaAppDTO>())
                .Any(p => p.Presente || p.AcompanhantePresente);
            var temProfissionalPresente = (profissionais ?? new List<PresencaProfissionalRotaAppDTO>())
                .Any(p => p.Presente);

            if (!rota.PermiteIniciarSemPacienteAcompanhante && !temPacienteOuAcompanhantePresente)
                throw new ApplicationException("Esta rota exige ao menos um paciente ou acompanhante presente para iniciar.");

            if (!rota.PermiteIniciarSemProfissional && !temProfissionalPresente)
                throw new ApplicationException("Esta rota exige ao menos um profissional presente para iniciar.");
        }
        ...
        private class RotaOfflineConsultaDTO
        {
            ...
            public bool PermiteIniciarSemPacienteAcompanhante { get; set; }
            public bool PermiteIniciarSemProfissional { get; set; }
        ...
        private class ParametrosInicioRotaDTO
        {
            public int Id { get; set; }
            public bool PermiteIniciarSemPacienteAcompanhante { get; set; }
            public bool PermiteIniciarSemProfissional { get; set; }
        ...
        private class RotaSincronizacaoOfflineLookupDTO : ParametrosInicioRotaDTO
        {
        ...
        private class RotaExecucaoPresencaLookupDTO
        {
            public int RotaId { get; set; }
            public int OrganizacaoId { get; set; }
        }

        private class RotaExecucaoFinalizadaHojeRowDTO
        {
            public int RotaId { get; set; }
